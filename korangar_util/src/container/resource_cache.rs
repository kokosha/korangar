...
use super::{Cacheable, GenerationalKey, Lru, SimpleIter, SimpleKey, SimpleSlab, Statistics, ValueTooBig};
...
impl<I: GenerationalKey + Copy, V: Cacheable> ResourceCache<I, V> {
    ...
    pub fn insert(&mut self, key: I, value: V) -> Result<(), ValueTooBig> {
        ...
        {
            if let Some((_, cache_key)) = self.cache.pop() {
    ...
    #[inline(always)]
    #[must_use]
    pub fn values(&self) -> SimpleIter<u32, V> {
        self.values.iter()
    }
    ...
    pub fn remove(&mut self, key: I) -> Option<V> {
        if let Some(fixed_key) = self.cache.remove(key) {
