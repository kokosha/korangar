...
impl<I: SimpleKey, T> SimpleSlab<I, T> {
    ...
    pub fn get(&self, key: I) -> Option<&T> {
        ...
        if let Some(Slot::Occupied(val)) = self.entries.get(key) {
            Some(val)
        } else {
            None
        }
    ...
    pub fn get_mut(&mut self, key: I) -> Option<&mut T> {
        let key = key.key() as usize;
        if let Some(Slot::Occupied(val)) = self.entries.get_mut(key) {
            Some(val)
        } else {
            None
        }
    ...
    pub fn insert(&mut self, value: T) -> Option<I> {
        if let Some(key) = self.next_free {
            if let Some(Slot::Empty(next_free)) = self.entries.get(key as usize) {
                self.next_free = *next_free;

                self.entries[key as usize] = Slot::Occupied(value);
                self.count += 1;

                return Some(I::new(key));
            }
    ...
    pub fn remove(&mut self, key: I) -> Option<T> {
        let i = key.key() as usize;
        if let Some(entry) = self.entries.get_mut(i) {
            if let Slot::Empty(_) = entry {
                return None;
            }

            let mut tmp = Slot::Empty(self.next_free);
            swap(entry, &mut tmp);
            self.next_free = Some(key.key());

            if let Slot::Occupied(val) = tmp {
                self.count -= 1;
                return Some(val);
            }
        }
    ...
    #[must_use]
    pub fn iter(&self) -> SimpleIter<I, T> {
        SimpleIter {
...
pub struct SimpleIter<'a, I, T: 'a> {
    entries: Enumerate<Iter<'a, Slot<T>>>,
    size: usize,
    _marker: PhantomData<I>,
}

impl<'a, I: SimpleKey, T> Iterator for SimpleIter<'a, I, T> {
    type Item = (I, &'a T);
...
mod tests {
    ...
    fn test_simple_slab_insert_and_get() {
        ...
        let key1 = slab.insert(10).unwrap();
        let key2 = slab.insert(20).unwrap();
        let key3 = slab.insert(30).unwrap();

        assert_eq!(slab.get(key1), Some(&10));
        assert_eq!(slab.get(key2), Some(&20));
        assert_eq!(slab.get(key3), Some(&30));
    ...
    fn test_simple_slab_remove() {
        ...
        assert_eq!(slab.remove(key1), Some(10));
        assert_eq!(slab.get(key1), None);
        assert_eq!(slab.count(), 1);

        assert_eq!(slab.remove(key2), Some(20));
        assert_eq!(slab.get(key2), None);
    ...
    fn test_secondary_simple_slab_insert_and_get() {
        ...
        let key1 = primary.insert(10).unwrap();
        let key2 = primary.insert(20).unwrap();

        secondary.insert(key1, "Hello".to_string());
        secondary.insert(key2, "World".to_string());

        assert_eq!(secondary.get(key1), Some(&"Hello".to_string()));
        assert_eq!(secondary.get(key2), Some(&"World".to_string()));
    }
    ...
    fn test_secondary_simple_slab_contains_key() {
        ...
        assert!(secondary.contains_key(key1));
        assert!(!secondary.contains_key(key2));
    }
    ...
    fn test_secondary_simple_slab_remove() {
        ...
        assert_eq!(secondary.remove(key1), Some("Hello".to_string()));
        assert_eq!(secondary.get(key1), None);
        assert_eq!(secondary.remove(key2), Some("World".to_string()));
