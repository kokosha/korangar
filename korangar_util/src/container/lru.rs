...
impl<I: GenerationalKey + Copy, V> Lru<I, V> {
    pub(crate) fn new(max_count: NonZeroU32) -> Self {
        ...
        (0..max_count.get() as isize).for_each(|i| unsafe {
            let node_ptr = cache.ptr.as_ptr().offset(i);
            node_ptr.write(Entry::new_zeroed());
            cache.attach_free(node_ptr);
        });
    ...
    pub(crate) fn touch(&mut self, key: I) {
        if let Some(node_ptr) = self.entries.get_mut(key).copied() {
    ...
    pub(crate) fn get(&self, key: I) -> Option<&V> {
        let node_ptr = self.entries.get(key)?;
        unsafe { (*(*node_ptr)).value.as_ref() }
    }
    ...
    pub(crate) fn remove(&mut self, key: I) -> Option<V> {
        let node_ptr = self.entries.remove(key)?;
        self.detach(node_ptr);
        self.attach_free(node_ptr);

        self.count -= 1;
        unsafe {
            self.size -= (*node_ptr).size;
            (*node_ptr).value.take()
        }
    }
    ...
    pub(crate) fn pop(&mut self) -> Option<(I, V)> {
        ...
        if prev != self.list {
            let key = unsafe { (*(*self.list).prev).key };
            if let Some(key) = key {
...
mod tests {
    ...
    fn test_put() {
        ...
        let x0 = slab.insert(false).unwrap();
        let x1 = slab.insert(false).unwrap();
    ...
    fn test_remove() {
        ...
        let x0 = slab.insert(false).unwrap();
        let x1 = slab.insert(false).unwrap();
        let x2 = slab.insert(false).unwrap();
    ...
    fn test_pop() {
        ...
        assert_eq!(cache.pop(), Some((x0, 1)));
        assert_eq!(cache.size(), 20);
        assert_eq!(cache.count(), 1);

        assert_eq!(cache.pop(), Some((x1, 2)));
        assert_eq!(cache.size(), 0);
        assert_eq!(cache.count(), 0);
    }
    ...
    fn test_random_remove() {
        ...
        let mut keys: Vec<TestKey> = (0..1_000)
            .map(|i| {
                let x = slab.insert(false).unwrap();
                cache.put(x, i, i);
                x
            })
            .collect();
    ...
    fn test_capacity_reached() {
        ...
        let mut cache: Lru<TestKey, usize> = Lru::new(NonZeroU32::new(2).unwrap());
        assert!(cache.put(x0, 1, 10));
        assert!(cache.put(x1, 2, 10));
        assert!(!cache.put(x2, 3, 10));
    }
}
