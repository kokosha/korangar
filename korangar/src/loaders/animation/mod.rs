...
impl Frame {
    /// The generate image will be overwrite in the order of the index of the
    /// vector
    //fn merge_rgba_image_data(vec_sprite_data: Vec<SpriteData>, vec_action_data:
    // Vec<ActionData>) -> RgbaImageData{
    pub fn merge_frame_part(mut vec_frame_part: Vec<FramePart>) -> RgbaImageData {
        ...
        let mut it_1 = vec_frame_part
            .iter()
            .max_by_key(|it| it.offset_x + it.rgba_data.width as i32)
            .unwrap();
        let mut it_2 = vec_frame_part
            .iter()
        ...
        /// Insert the images in the new image in the order of the index of the
        /// vector
